...
# -*- coding: utf-8 -*-
"""
Created on Sun May 12 05:54:58 2024
...
def model_basic(t, y, pars, Vunstr, T):
    ...
    # Unpack parameters
    Rs, Rp, Rpv, Rsv, Csv, Cpa, Cpv, Emra, Emla, EMrv, Emrv, Emlv, tcra, Tcra = pars
...
parameters = [
    0.86, 0.036, 0.036, 0.072,  # Resistances Rs, Rp, Rpv, Rsv
    31.2, 7.0, 11.6,  # Compliances Csv, Cpa, Cpv
    0.04, 0.04, 0.34, 0.021, 0.055,  # Elastance Emra, Emla, EMrv, Emrv, Emlv
    0.8, 0.97 # Timing tcra, Tcra
]
...
def run_model(params, output_index):
    ...
    Rs, Rp, Rpv, Rsv, Csv, Cpa, Cpv, Emra, Emla, EMrv, Emrv, Emlv, tcra, Tcra = params
    
    # Prepare parameters for the model, can be a dictionary or directly as arguments
    model_params = [
        Rs, Rp, Rpv, Rsv, Csv, Cpa, Cpv, Emra, Emla, EMrv, Emrv, Emlv, tcra, Tcra
    ]
...
fixed_params = {'Tcrv': 0.3, 'EMla': 0.85, 'EMra': 0.79, 'Trra': 0.1, 'Rtva': 0.003, 'Rmva': 0.003, 'Rpva': 0.0025, 'Trrv': 0.51, 'Rava': 0.014, 'Csa': 1.8, 'EMlv': 1.92}
param_names = [name for name in initial_values.keys() if name not in fixed_params]
bounds = [[initial_values[name] * (1 - rc), initial_values[name] * (1 + rc)] for name in param_names if name not in fixed_params]
...
if __name__ == '__main__':
    ...
    output_labels = ['Vsa', 'Vsv', 'Vpa', 'Vpv', 'Vra', 'Vla', 'Vrv', 'Vlv']
